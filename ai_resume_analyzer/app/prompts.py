...
Your response must be a valid JSON object.
Use `null` if a value is missing.
If experience is not available as a range, for example, "5+years", consider it as 5 for min_work_experience and 8 for max_work_experience (assuming +3 years as the maximum).
...
- Whether the work experience is within the acceptable range. This should be between the min_work_experience and max_work_experience from the job description with a tolerance of +/- 2 years. Anything outside this range should be considered a mismatch and the candidate should be rejected.
